...
def call(script: str, args: list[str], optional: bool = False) -> bool:
    ...
    try:
        ...
        if optional:
            print(f"[MAIN] Optional stage failed; continuing: {script} (exit {exc.returncode})", flush=True)
            return False
        raise
...
def main() -> int:
    ...
    p.add_argument("--strict-diarize", action="store_true", help="Fail if speaker diarization is unavailable")
    p.add_argument("--force", action="store_true", help="Delete the existing workflow output before starting")
    p.add_argument("--live", action="store_true", help="Print live results from stages that support it")
    args = p.parse_args()

    video = args.video.resolve()
    out = (args.output_dir or video.with_name(video.stem + "_transcript")).resolve()
    if args.force and out.exists():
        print(f"[MAIN] Removing {out}", flush=True)
        shutil.rmtree(out)
    out.mkdir(parents=True, exist_ok=True)

    audio = out / "audio_16k_mono.wav"
    transcript = out / "transcript.json"
    aligned = out / "aligned.json"
    diar = out / "diarization.json"
    ...
    if args.stage in ("all", "transcribe"):
        ...
        if args.compute_type:
            transcribe_args += ["--compute-type", args.compute_type]
        if args.live:
            transcribe_args += ["--live"]
    ...
    if args.stage in ("all", "align"):
        call(
            ...
            [
                ...
                *( ["--compute-type", args.compute_type] if args.compute_type else [] ),
            ],
        )

    if args.stage in ("all", "diarize") and not args.skip_diarize:
        diar_args = ["--audio", str(audio), *common]
        if args.strict_diarize:
            diar_args += ["--strict"]
        if args.live:
            diar_args += ["--live"]
        call("diarize.py", diar_args, optional=not args.strict_diarize)

    if args.stage in ("all", "verify"):
        call("verify.py", ["--transcript", str(transcript), *common])

    if args.stage in ("all", "export"):
        source = aligned if aligned.exists() else transcript
        ex = ["--input", str(source), *common]
        if diar.exists():
            ex += ["--diarization", str(diar)]
